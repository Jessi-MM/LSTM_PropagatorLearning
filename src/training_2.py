...
from validation_functions import long_rollout, log_density_plot # validation functions to plot real predicted trajectories
import time
...
for epoch in range(num_epochs):
    ...
    val_loss, pred_trajs, true_trajs = long_rollout(model, val_loader, device)
    print(f"Epoch [{epoch+1}/{num_epochs}] Train Loss: {epoch_loss:.6f} | Rollout Val Loss: {val_loss:.6f}")

    # Log density plot (once per epoch, using epoch index as step)
    if len(pred_trajs) > 0:  # make sure we have at least one trajectory
        traj_idx = 0
    ...
    writer.add_scalar('Loss/Rollout_Val', val_loss, epoch)

    # --- Optional best saving ---
